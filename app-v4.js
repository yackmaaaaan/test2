...
function parseDet(t,meta){
  ...
  log('=== detector parser v4 ===');
  log('det output shape',JSON.stringify(sh));
  if(sh.length!==3) throw new Error('未対応の検出出力形状: '+JSON.stringify(sh));

  // Ultralytics YOLO26 segmentation raw output:
  // [1, 4 + nc + nm, 8400]. This model has nc=1 (tile), nm=32,
  // therefore 37 = bbox 4 + tile score 1 + mask coefficients 32.
  // channel 4 is already the class score. DO NOT apply sigmoid again.
  let N,C,at;
  ...
  let scores=[];
  for(let n=0;n<N;n++) scores.push(at(n,4));
  scores.sort((a,b)=>b-a);
  log('score mode','raw class probability (no sigmoid)');
  log('top scores',scores.slice(0,25).map(v=>Number(v).toFixed(3)).join(', '));
  ...
  for(let n=0;n<N;n++){
    ...
    let x=at(n,0), y=at(n,1), w=at(n,2), h=at(n,3);
    ...
    if(bw>imgBitmap.width*.30 || bh>imgBitmap.height*.45) continue;
    const ar=bw/bh;
    // Upright tile faces are taller than wide. Keep some perspective tolerance.
    if(ar<0.28 || ar>1.15) continue;
    rows.push({x1,y1,x2,y2,score,bw,bh,cx:(x1+x2)/2,cy:(y1+y2)/2});
  }
  log('candidates >= '+threshold+' + geometry',rows.length);
  ...
  let keep=[];
  for(const r of rows) if(!keep.some(k=>iou(k,r)>.45)) keep.push(r);
  log('after NMS',keep.length);

  // Tile YOLO was trained for region images. When a full phone photo is supplied,
  // select the strongest horizontal row of similarly-sized upright detections.
  if(keep.length>2){
    let best=[];
    for(const seed of keep){
      const tol=Math.max(seed.bh*0.65, imgBitmap.height*0.025);
      const cluster=keep.filter(r=>Math.abs(r.cy-seed.cy)<=tol && r.bh/seed.bh>.45 && r.bh/seed.bh<2.2);
      const quality=cluster.length*10 + cluster.reduce((z,r)=>z+r.score,0);
      const bestQuality=best.length ? best.length*10 + best.reduce((z,r)=>z+r.score,0) : -1;
      if(quality>bestQuality) best=cluster;
    }
    if(best.length>=2){ keep=best; log('horizontal row selected',keep.length); }
  }

  keep.sort((a,b)=>a.cx-b.cx);
  if(keep.length>18){
    keep=keep.sort((a,b)=>b.score-a.score).slice(0,18).sort((a,b)=>a.cx-b.cx);
  }
  log('final detected',keep.length);
  return keep;
}
